	...
	function ScheduleTemplate( element ) {
		...
		this.modalClose = this.modal.getElementsByClassName('schedule-modal__close')[0];
		...
		this.Form = this.element.getElementsByClassName('editEvent')[0];
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		this.Form.setAttribute('style', 'opacity:1;');
		...
		if( mq == 'mobile' ) {
			...
			self.modalBody.setAttribute('style', 'background-color: white; margin-left:'+eventWidth+'px');
	...
	ScheduleTemplate.prototype.closeModal = function() {
		...
		if( mq == 'mobile' ) {
			...
			this.Form.setAttribute('style', 'opacity:0;');
			var eventPosition = target.getBoundingClientRect(),
				eventTop = eventPosition.top,
				eventLeft = eventPosition.left,
				eventHeight = target.offsetHeight,
		...
		var route = "/calendar/action"
						   ...
						   console.log(data)
							for (let index = 0; index < data.length; index++) {
								...
								'<li class="cd-schedule__event">\
									  <a data-start="'+data[index].start+'" data-id="'+data[index].id+'" data-end="'+data[index].end+'"  data-content="'+data[index].title+'" data-event="event-'+data[index].day+'" href="#0">\
										<em class="cd-schedule__name">'+data[index].title+'</em>\
									  </a>\
									</li>';
	
									(data[index].day==1)?$("#Monday").append(event):(data[index].day==2)?$("#Tuesday").append(event):(data[index].day==3)?$("#Wednesday").append(event):(data[index].day==4)?$("#Thursday").append(event):(data[index].day==5)?$("#Friday").append(event):(data[index].day==6)?$("#Saturday").append(event):(data[index].day==7)?$("#Sunday").append(event):null;
				...
				scheduleTemplateArray[i].placeEvents();
					   ...
					   complete: function(){
						...
						$('.cd-schedule__event').on('click',function(){
							
									   $.ajax({
										   url: config.routes.editSeance,
										   type: 'get',
										   data: {
						
											   id :$(this).find('a:first').attr('data-id'),
						} 
						,
										   dataType : 'json',
										   success: function (data){
											   console.log(data.Seance)
											$('#editEvent').find('option[value='+data.Seance.jour+']').attr('selected','selected')
											$('#editEvent').find('input[name="heureSeance"]').val(data.Seance.heure)
											$('#editEvent').find('input[name="dureeSeance"]').val(data.Seance.duree)
											$('#editEvent').find('input[name="seanceId"]').val(data.Seance.id)
											$('#editEvent').find("select[name='salle']").html(' <option selected value="'+data.Seance.salle_id+'">'+data.Salle.label+'</option>')
											$('#editEvent').find("select[name='matiere']").html(' <option selected value="'+data.Seance.matiere_id+'">'+data.Matiere.intitule_matiere+'</option>')
											$('#editEvent').find("select[name='professeur']").html(' <option selected value="'+data.Seance.professeur_id+'">'+data.Professeur.nom+' '+data.Professeur.prenom+'</option>')
